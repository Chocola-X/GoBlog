...
程序会在 `data/gopherink.db` 创建 SQLite 数据库，监听 `:8080`，默认管理员账号 `admin` / `admin123`。

访问 `http://localhost:8080/admin/` 进入后台。

### Web 安装向导

首次启动时，如果未设置管理员环境变量且数据库为空，程序会启用 Web 安装向导（`GOPHERINK_WEB_INSTALL=true`），可通过浏览器完成站点初始化和管理员账户创建。

### 环境变量配置

| 变量 | 说明 | 默认值 |
|------|------|--------|
| `GOPHERINK_ADDR` | 监听地址 | `:8080` |
| `GOPHERINK_DB_DRIVER` | 数据库驱动（`sqlite3`/`mysql`/`mariadb`/`postgres`） | 交互选择 |
