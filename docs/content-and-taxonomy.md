...
后台编辑器通过独立自动保存入口提交当前表单。服务端根据当前内容 ID 和 `published_id` 判断更新未发布草稿还是已发布内容的编辑草稿。
...
修订快照保存在 `gb_revisions`，与编辑草稿分离。修订记录包括标题、别名、正文、状态、密码、排序、模板、父级和三个内容开关等可恢复字段。

- 默认每项内容最多保留 20 条。
- 后台可配置修订上限，服务层限制为 1 到 1000。
- 可以查看全文、恢复或删除单条修订。
- 删除内容时同步删除它的修订。
