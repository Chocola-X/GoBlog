...
插件路由和生命周期会收到 `*plugin.Runtime`，当前提供：
...
`PublicContent` 是供插件使用的精简结构，不等同于 `models.Content`。`ContentByID` 底层按内部 ID 读取，可能返回草稿、隐藏内容或附件；公开插件路由必须检查 `Type`、`Status` 并建立自己的授权边界，不能把“返回 PublicContent”理解成“已经确认公开”。需要核心服务模型的钩子 payload 会明确携带当前具体类型。
...
- 需要完整后台安全上下文的插件页面，在核心提供正式插件后台认证包装接口前，不应处理敏感配置写入。
...
func (Plugin) AdminMenuItems(ctx context.Context) []plugin.AdminMenuItem {
    return []plugin.AdminMenuItem{
        {Label: "示例工具", URL: "/example/status", Icon: "extension"},
    }
}
```

`Icon` 使用后台 Material Icon 名称，留空时界面使用默认 `extension`。菜单只负责一致的导航外观，不为目标 URL附加权限。敏感页面仍受上一节限制。
