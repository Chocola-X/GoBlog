...
- 扩展存储、后台页面和命名服务：替代插件直接扩展核心表的做法。高频或大体量数据使用插件/主题独占 SQLite，跨插件/主题调用使用命名服务。

能由更底层的通用接口完整实现的能力，不再增加同义专用钩子。例如访客统计使用请求生命周期和独占 SQLite，不需要“访客统计钩子”。不能等价替代的状态变化仍保留业务钩子，例如内容发布前后、评论审核前后和附件存储处理。
...
认证 payload 不包含明文密码。登录成功后的用户信息使用 `PublicUser`，不暴露密码哈希、认证版本或会话密钥。
...
阅读计数、访问统计等插件数据应写入插件独占 SQLite，而不是写入核心内容字段：
...
func countView(rt *plugin.Runtime, w http.ResponseWriter, r *http.Request) {
	...
	db, _, err := rt.OpenSQLiteDatabase(r.Context(), "views.db")
	if err != nil {
	    http.Error(w, "counter unavailable", http.StatusInternalServerError)
	    return
	}
	_, _ = db.ExecContext(r.Context(), `CREATE TABLE IF NOT EXISTS views (
	    cid INTEGER PRIMARY KEY,
	    count INTEGER NOT NULL DEFAULT 0
	)`)
	_, err = db.ExecContext(r.Context(), `INSERT INTO views (cid, count) VALUES (?, 1)
	    ON CONFLICT(cid) DO UPDATE SET count = count + 1`, cid)
	if err != nil {
	    http.Error(w, "counter unavailable", http.StatusInternalServerError)
	    return
	}
	var views int64
	_ = db.QueryRowContext(r.Context(), `SELECT count FROM views WHERE cid = ?`, cid).Scan(&views)
	_ = json.NewEncoder(w).Encode(map[string]int64{"views": views})
}
```

公开计数接口仍应考虑 WAF、去重和滥用；独占数据库只解决插件数据隔离，不验证一次访问的真实性。
...
这类插件不需要也不应依赖“访客记录专用钩子”。`request.before/request.after`、`Runtime.ClientIP`、`OpenSQLiteDatabase` 和后台页签已经覆盖请求采集、存储和展示。
